...
export class UsersService extends PrismaClient implements OnModuleInit{
  ...
  async findAll(paginationDto:PaginationDto) {
    ...
    const totalRegistry=await this.usuariosCP.count({
    ...
    return {
      data:await this.usuariosCP.findMany({
        skip:(page-1)*limit,
        take:limit,
        //where:{available:true}
      }),
  ...
  async findOne(id: number) {
    const product=await this.usuariosCP.findFirst({where:{id}
    });
    if(!product){
  ...
  async remove(id: number) {
    await this.findOne(id);
    const product =await  this.usuariosCP.delete({
      where:{id},
